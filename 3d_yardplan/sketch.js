...
let maxRow = 0,
  maxBay = 0,
  maxTier = 0; // Số Bay, row, tier lớn nhất
...
const states = [
  // Cấu hình camera mặc định
  {
    ...
    center: [561.3654397889591, -270.0896240255795, 695.8011306993076],
    rotation: [
      -0.8513564699917174, 0.06907587081447907, 0.5186968187780248,
      -0.03707148855939367,
    ],
  },
];
...
// p5js main functions
function preload() {
  // Hàm load trước dữ liệu vào 2 biến cArray và depot

  let dpName = ["ETD", "CSD", "TBD", "CLD", "CPD", "CTC", "GKP"];
  async function load() {
    // depot = await getDepotConfigByDepotID("32");
    // console.log(depot);
    // cArray = await getContArray("CLD");
    // console.log(cArray);
    depot = await $.getJSON("../../yardplan_creator/data1/etdv2.json");
    // console.log(depot);
    cArray = await $.getJSON("./data/cont3.json");
    // console.log(depot);
...
function setup() {
  // Cài đặt cơ bản của app
  createCanvas(windowWidth - 10, windowHeight - 100, WEBGL);
  noLoop();
}

function draw() {
  // Vòng lặp để vẽ cho mỗi frame
...
function drawCont(cont) {
  ...
  try {
    var d = createVector(
      depot.Area[area].x_coor + depot.contLength,
      depot.Area[area].y_coor + depot.contWidth,
      0.5 * depot.contHeight + 20
    );
  } catch (error) {
    console.log(area);
  }
  let k = createVector(
    -r * depot.contWidth,
    b * (depot.contLength + depot.contGap)
  );
  k.rotate(-depot.Area[area].angle);
  let d2 = p5.Vector.add(d, k);
  let eye = createVector(
    eyeVector[0] + depot.center.x,
    eyeVector[2] + depot.center.y,
    -eyeVector[1]
  );
  ...
  try {
    translate(
      0.5 * depot.contLength,
      -0.5 * depot.contWidth * x_flip,
      0.5 * depot.contHeight + 10
    );
...
function drawDepot() {
  ...
  for (let i = 0; i < depot.Area.length; i++) {
    ...
    translate(
      depot.Area[i].x_coor - depot.contLength,
      depot.Area[i].y_coor,
      11
    );
  ...
  for (let i = 0; i < house.length; i++) {
    if (house[i].type < 2) {
      ...
      for (let j = 0; j < house[i].shape.seq.length - 1; j++) {
        ...
        vertex(
          house[i].shape.seq[j + 1].x,
          house[i].shape.seq[j + 1].y,
          house[i].height + 10
        );
        vertex(
          house[i].shape.seq[j].x,
          house[i].shape.seq[j].y,
          house[i].height + 10
        );
        endShape(CLOSE);
      }
...
function drawSideCont(cont, twenty_feet) {
  // Vẽ tên cont lên hai đầu cont
  push();
  if (!twenty_feet) {
    translate(0, 0, depot.contLength + 1);
    textSize(depot.contHeight / 5);
    text(
      cont.ContID.substring(0, 4) + "\n" + cont.ContID.substring(4, 11),
      0,
      0
    );
    rotateY(Math.PI);
...
function setColor(opt) {
  // Hàm set giá trị màu cho cont theo tên hãng tàu
  stroke(2);
  switch (opt) {
    case "HLC":
      fill(252, 140, 3);
...
function findCenter() {
  // Tìm toạ độ điểm trung tâm của depot
  var minX = Infinity,
    maxX = -Infinity;
  var minY = Infinity,
    maxY = -Infinity;
...
function processCont() {
  // Hàm đổ cont từ cArray qua contArray3D + tìm max bay, row, tier
  for (let i = 0; i < cArray.length; i++) {
...
const deviceType = () => {
  // Hàm tìm loại thiết bị của người dùng
...
function checkKeyPress() {
  // Kiểm tra nút đang bấm trên bàn phím
  currentState = center;
  if (keyIsPressed) {
    dirVec = heading;
...
function windowResized() {
  // Bắt sự kiện thay đổi kích thước trình duyệt web
...
function changeTextVisibility() {
  // Sự kiện bấm nút ẩn hiện tên cont
...
function rotateDiff(a, r) {
  // Tính Vector chênh lệnh sau khi xoay vector ban đầu
  let c = a.copy();
  a.rotate(r);
...
function getAng(quat) {
  // Chuyển đổi từ quaternion ra 3 góc
  x = quat[0];
  y = quat[1];
  z = quat[2];
  w = quat[3];
...
function dirVector(quat) {
  // Tính vector 2D cơ bản từ quaternion
  eye = getAng(quat);
  let V = Array(2);
  V[1] = Math.cos(eye[0]);
  V[0] = Math.sin(eye[0]);
