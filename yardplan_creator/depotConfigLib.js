...
function drawDepot() {
  ...
  for (let i = 0; i < depot.layout.line.length; i++) {
    line(
      depot.layout.line[i].p1.x,
      depot.layout.line[i].p1.y,
      depot.layout.line[i].p2.x,
      depot.layout.line[i].p2.y
    );
  }
  pop();
}
...
function drawGrid() {
  ...
  if (document.getElementById("checkAngle").checked == false) {
    for (let x = 0; x < ground[activeGround].wid; x += depot.contWidth) {
      for (
        let y = 0;
        y < ground[activeGround].hei;
        y += depot.contLength + depot.contGap
      ) {
    ...
    for (
      let x = 0;
      x < ground[activeGround].wid;
      x += depot.contLength + depot.contGap
    ) {
...
function drawSelectionRect() {
  if (mouseIsPressed && selectRectStart != undefined) {
    ...
    let dif = rotateDiff(
      createVector(mouseX - selectRectStart.x, mouseY - selectRectStart.y),
      ground[activeGround].angle
    );
    //
    ...
    rect(
      0,
      0,
      mouseX - selectRectStart.x + dif.x,
      mouseY - selectRectStart.y + dif.y
    );
    pop();
  }
}
...
function drawSelection() {
  ...
  for (let i = 0; i < selection.length; i++) {
    if (!gridAngle) {
      rect(
        selection[i].x * depot.contWidth,
        selection[i].y * (depot.contLength + depot.contGap),
        depot.contWidth,
        depot.contLength
      );
    } else {
      rect(
        selection[i].x * (depot.contLength + depot.contGap),
        selection[i].y * depot.contWidth,
        depot.contLength,
        depot.contWidth
      );
    }
  ...
  for (let i = teuArray.length - 1; i >= 0; i--) {
    ...
    if (teuArray[i].orient == 0) {
      ...
      if (scaleFactor > 1) {
        ...
        noStroke();
        text(
          teuArray[i].num_of_tier,
          p.x + depot.contWidth / 2,
          p.y + depot.contLength / 2
        );
        // textSize(10)
      ...
      if (scaleFactor > 1) {
        ...
        text(
          teuArray[i].num_of_tier,
          p.x + depot.contLength / 2,
          p.y + depot.contWidth / 2
        );
...
function drawLine() {
  ...
  for (let i = 0; i < depot.Area.length; i++) {
    ...
    stroke("RED");
    let l =
      depot.Area[i].num_bay * depot.contLength +
      (depot.Area[i].num_bay - 1) * depot.contGap;
...
function drawCursor() {
  ...
  translate(width / 2 + depot.offset.x, height / 2 + depot.offset.y);
  translate(
    ground[activeGround].offsetX * scaleFactor,
    ground[activeGround].offsetY * scaleFactor
  );
  rotate(-ground[activeGround].angle);
  scale(scaleFactor);
  strokeWeight(1);
...
function mouseDragged() {
  if (mouseButton === LEFT) {
    ...
    blank = createVector(
      (width - depot.width * scaleFactor) / 2,
      (height - depot.height * scaleFactor) / 2
    );
...
function mouseMap(i, j) {
  ...
  // let y = mm2.y;
  let p = createVector(
    i - width / 2 - depot.offset.x,
    j - height / 2 - depot.offset.y
  );
...
function mouseMap2(x, y) {
  ...
  let v1 = createVector(
    ground[activeGround].offsetX,
    ground[activeGround].offsetY
  );
...
function gridMaping(px, py) {
  ...
  if (!gridAngle) {
    ...
    return [
      new Point(dx, dy),
      new Point(
        Math.round(dx - blank.x / (depot.contWidth * scaleFactor)),
        Math.round(dy - blank.y / (depot.contLength * scaleFactor))
      ),
    ];
  }
...
function gridMapingTranspose(p, or = 1) {
  if (!or) {
    return new Point(
      p.x * depot.contWidth,
      p.y * (depot.contLength + depot.contGap)
    );
  } else {
    return new Point(
      p.x * (depot.contLength + depot.contGap),
      p.y * depot.contWidth
    );
  }
}
...
function initTeuArray() {
  // chua toi uu height, width cua ground
  for (let g = 0; g < ground.length; g++) {
    ...
    for (let x = 0; x < ground[g].wid / depot.contWidth; x++) {
      ...
      for (
        let y = 0;
        y < ground[g].hei / (depot.contLength + depot.contGap);
        y++
      ) {
    ...
    for (
      let x = 0;
      x < ground[g].wid / (depot.contLength + depot.contGap);
      x++
    ) {
      let temp = [];
...
function updatePanel(teu) {
  if (
    teu.bay_name === undefined ||
    teu.opt === undefined ||
    teu.num_of_tier === undefined
  ) {
    ...
    document.getElementById("check_1mat").checked =
      depot.Area[areaIndex].one_face;
    document.getElementById("check_x_flip").checked =
      depot.Area[areaIndex].x_flip;
    document.getElementById("check_y_flip").checked =
      depot.Area[areaIndex].y_flip;
...
function initButton() {
  ...
  for (let i = 0; i < ground.length; i++) {
    let p1 = createVector(
      ground[i].button.position.x,
      ground[i].button.position.y
    );
    ...
    buttonArray.push(
      new Button(
        p2.x,
        p2.y,
        ground[i].button.text,
        i,
        -ground[i].button.angle,
        scaleFactor
      )
    );
  }
...
function findAreaOrigin(area) {
  ...
  for (let t = 0; t < teuArray.length; t++) {
    if (teuArray[t].bay_name == area) {
      minX = Math.min(minX, teuArray[t].x);
      minY = Math.min(minY, teuArray[t].y);
      maxX = Math.max(maxX, teuArray[t].x);
      maxY = Math.max(maxY, teuArray[t].y);
      g = teuArray[t];
    }
  }
  return {
  ...
  try {
    if (!gridAngle) {
      if (
        p1.x >= 0 &&
        p1.y >= 0 &&
        ground[activeGround].verticalArray[p1.x][p1.y].opt != undefined
      )
        return ground[activeGround].verticalArray[p1.x][p1.y];
      if (
        p2.x >= 0 &&
        p2.y >= 0 &&
        ground[activeGround].horizontalArray[p2.x][p2.y].opt != undefined
      )
        return ground[activeGround].horizontalArray[p2.x][p2.y];
    } else {
      if (
        p1.x >= 0 &&
        p1.y >= 0 &&
        ground[activeGround].verticalArray[p2.x][p2.y].opt != undefined
      )
        return ground[activeGround].verticalArray[p2.x][p2.y];
      if (
        p1.x >= 0 &&
        p1.y >= 0 &&
        ground[activeGround].horizontalArray[p1.x][p1.y].opt != undefined
      )
        return ground[activeGround].horizontalArray[p1.x][p1.y];
    }
  } catch (error) {}
  ...
  if (bayNameArray.indexOf(bay.toUpperCase()) < 0) {
    ...
    for (let i = 0; i < selection.length; i++) {
      ...
      if (bayNameArray.indexOf(bay.toUpperCase()) >= 0) {
        if (!gridAngle) {
          if (
            ground[activeGround].verticalArray[x - 1][y].bay_name ==
              bay.toUpperCase() ||
            ground[activeGround].verticalArray[x][y - 1].bay_name ==
              bay.toUpperCase()
          ) {
            ground[activeGround].verticalArray[x][y].opt = opt.toUpperCase();
            ground[activeGround].verticalArray[x][y].bay_name =
              bay.toUpperCase();
            ground[activeGround].verticalArray[x][y].num_of_tier = tier;
            ground[activeGround].verticalArray[x][y].orient = gridAngle;
            ground[activeGround].verticalArray[x][y].ground = activeGround;
          ...
          if (
            ground[activeGround].horizontalArray[x - 1][y].bay_name ==
              bay.toUpperCase() ||
            ground[activeGround].horizontalArray[x][y - 1].bay_name ==
              bay.toUpperCase()
          ) {
            ground[activeGround].horizontalArray[x][y].opt = opt.toUpperCase();
            ground[activeGround].horizontalArray[x][y].bay_name =
              bay.toUpperCase();
            ground[activeGround].horizontalArray[x][y].num_of_tier = tier;
            ground[activeGround].horizontalArray[x][y].orient = gridAngle;
            ground[activeGround].horizontalArray[x][y].ground = activeGround;
...
function resetArea() {
  for (let i = 0; i < selection.length; i++) {
    if (!gridAngle) {
      ground[activeGround].verticalArray[selection[i].x][selection[i].y].opt =
        undefined;
      ground[activeGround].verticalArray[selection[i].x][
        selection[i].y
      ].bay_name = undefined;
      ground[activeGround].verticalArray[selection[i].x][
        selection[i].y
      ...
      ground[activeGround].horizontalArray[selection[i].x][selection[i].y].opt =
        undefined;
      ground[activeGround].horizontalArray[selection[i].x][
        selection[i].y
      ].bay_name = undefined;
      ground[activeGround].horizontalArray[selection[i].x][
        selection[i].y
    ...
    if (!origin.orient) {
      newArea.push(
        new Area(
          ...
          x,
          y,
          ground[origin.ground].angle,
          depot.Area[i].x_flip,
          depot.Area[i].y_flip,
          depot.Area[i].one_face,
          origin.hei,
          origin.wid,
          depot.Area[i].orient
        )
      ...
      newArea.push(
        new Area(
          ...
          x,
          y,
          ground[origin.ground].angle + PI / 2,
          depot.Area[i].x_flip,
          depot.Area[i].y_flip,
          depot.Area[i].one_face,
          origin.wid,
...
async function changeDepot() {
  ...
  depot = await $.getJSON("./data1/etdv2.json");
  teuArray = await $.getJSON("./data1/etdv2_reservation.json");
  ground = depot.ground;
